...
For any $p > 0$ define $L_p(\mathbb{T})$ to be the linear space of measurable, complex-valued functions $\phi: \mathbb{T} \rightarrow \mathbb{C}$ such that 
...
Recall _Hölder's inequality_.  For each $p \in (1, \infty)$ define $q = p / (p-1)$ to be the Hölder conjugate index of $p$.  Then 
...
Extend this set of conjugate pairs to include $(1, \infty)$.  Then for $f \in L_p(\mathbb{T})$ and $g \in L_q(\mathbb{T})$, the point-wise product $f(\theta) \times g(\theta)$ is a member of $L_1(\mathbb{T})$ and satisfies Hölder's inequality:
...
In general, an element of $\mathcal{C}(\mathbb{T})^*$ can be represented by a complex-valued, finitie measure $\mu$ on $\mathbb{T}$ so that 
...
\begin{align}
  ...
  \\ 
  \mu_d (E) &= \sum_{ \tau \in E \; \cap \; \sigma(\mu_d) } \rho_\tau \; e^{i \theta_tau}
\end{align}
$$
...
The linear functions $\lambda_\mu$ evaluated at $f(\cdot)$ is thus 

$$
\begin{align}
  \lambda_\mu (f) &= \int_\mathbb{T} f(\theta_\omega) \; g(\theta_\omega) \; d\mu (\omega) \\ 
  &= \frac{1}{2 \pi} \int_\mathbb{T} f(\theta) \; g(\theta) \; d\theta \; 
...
where $\mathcal{M} (\mathbb{T})$ denotes teh Banach space of finite, complex-valued Borel measures $\mu (\cdot)$ defined on $\mathbb{T}$.
...
The functions $\left \{ W_n(\cdot) \right \}$ may be constructed to satisfy the requirements of a family of _kernel functions_: integrating to unit, and concentrated on a vanishingly small neighborhood of zero as $n \rightarrow \infty$.  If $W_n * f$ converges in norm to $f$, for all $f \in L_1 (\mathbb{T})$, the sequence $\left \{ W_n(\cdot) \right \}$ is said to be an approximate identity.
...
The first term $\hat{g}_\mu [\nu]$ tends to zero as $| \nu |$ increases.  If measure $\mu$ has one or more atoms $\tau$ then the second term is non-zero and generates a sequence $\{ \hat{\mu}_d [\nu] \}$.  This sequence is a sampling of a periodic or almost-periodic function that does not converge to zero, but is bounded by $\lVert \mu_d \rVert = \sum \rho_t$.
...
So far we've constructed a framework in which any set of bounded coefficients can be interpreted as a set of Fourier coefficients.  Now we want to set the stage for a subsequent technical note in which a set of coefficients will play the central role of a time series.

Time series are usually defined as a sequence of random variables having certain properties, but in this section we present equivalent analytic properties of a non-random sequence of coefficients, say $\xi_\bullet = \{ \xi_k \}$.

First, we require the sequence to have a "time-averaged" limiting value.
...
Note that we require the above summation limits $m, n$ to tend to infinity independently.  That is, for a given $\epsilon > 0$ there must exist $M(\epsilon), N(\epsilon)$ such that _any_ $(m, n)$-average, with $m \ge M(\epsilon)$ and $n \ge N(\epsilon)$ is within $\epsilon$ of its limiting value.

We also require the following quadratic forms to have time-averaged limiting values.
...
The requirement that these limits hold however $m, n$ may approach infinity ensures that the same limits hold for any translation of a given series, that is, for $\xi_\nu^\prime = \xi_{\nu + \tau}$, where $\tau$ is any fixed integer.  This consequent translation-invariance of the limits corresponds, in the theory of random time series, to an assumption of "second-order stationarity".

We also impose a so-called "mixing" requirement that a time series be asymptotically uncorrelated with a sufficiently large translation of itself:
...
If a time series $\xi_\bullet = \{ \xi_|nu \}$ meets these mixing and translation-invariant requirements, then there is a Borel measure $\mu_\xi$ defined on $\mathbb{T}$ such that 

$$
\begin{align}
  \xi_\nu &= \int_\mathbb{T} \bar{u}_\nu (\omega) \; d\mu_\xi(\omega) \\ 
  &= \int_{- \pi}^\pi e^{-i \nu \theta} \; d\mu_\xi(e^{i \theta}) \\ 
  &= \hat{\mu}_\xi [\nu]
\end{align}
$$

We previously introduced Borel measures as linear functionals on $\mathcal{C}(\mathbb{T})$, the Banach space of continuous functions defined on $\mathbb{T}$.  But the norms and constraints are now different.
...
This persistent error at jump discontinuities is known a Gibbs phenomenon, named after Josiah Willard Gibbs.
...
Since $D_n (\cdot)$ is concentrated near 0 mod $2 \pi$ and integrates to unity on $\mathbb{T}$, $S_n (f) (\theta)$ converges to $f (\theta)$ if $f (\cdot)$ is continuous in a neighborhood of $\theta$.  But as previuosly noted, if $f (\cdot)$ has a jump discontinuity at $\theta$, then for all $n$, $S_n (f) (\theta)$ incurs an error near $\theta$ whose magnitude is approximately 9% of the magnitude of the jump (Gibbs phenomenon).
