...
This technical note introduces the Fourier series approximation of $2 \pi$-periodic functions as a starting point for the Fourier analysis of time series (to be introduced in a separate technical note).
...
$$
\begin{align}
  \phi \in L_p(\mathbb{T}) &\Leftrightarrow \int_\mathbb{T} | \phi |^p \; d\lambda_\mathbb{T} < \infty
\end{align}
$$
...
$$
\begin{align}
  {\lVert \phi \rVert}_p &= \left ( \int_\mathbb{T} | \phi |^p d\lambda_\mathbb{T} \right )^{ \frac {1}{p}}
\end{align}
$$

Note: For $0 < p < 1$, ${\lVert \phi \rVert}_p$ fails to be sub-additive, and thus does not qualify as a norm.
...
$$
\begin{align}
  b \in U_\phi &\Leftrightarrow \lambda_\mathbb{T} \left ( \{ \omega: | \phi (\omega) | > b \} \right ) = 0 
\end{align}
$$
...
This norm is called the _essential upper bound_ of $| \phi(\cdot) |$.  If ${\lVert \phi \rVert}_\infty < \infty$, then $\phi(\cdot)$ is said to be _essentially bounded_.  The normed linear space of essentially bounded functions is denoted $L_\infty(\mathbb{T})$.
...
Since $\lambda_\mathbb{T}(\mathbb{T})$ is finite (and moreover equal to unity), we can show that for all finite $p > 0$ we have ${\lVert \phi \rVert}_p \le {\lVert \phi \rVert}_\infty$.

$$
\begin{align}
  {\lVert \phi \rVert}_p &= \left ( \int_\mathbb{T} | \phi (\omega) |^p \; d\lambda_\mathbb{T} (\omega) \right )^{ \frac {1}{p}} \\ 
  &\le \left ( \int_\mathbb{T} {\lVert \phi \rVert}_\infty^p \; d\lambda_\mathbb{T} (\omega) \right )^{ \frac {1}{p}} \\ 
  &= {\lVert \phi \rVert}_\infty \left ( \int_\mathbb{T} 1 \; d\lambda_\mathbb{T} (\omega) \right )^{ \frac {1}{p}} \\ 
  &= {\lVert \phi \rVert}_\infty \; \left ( \lambda_\mathbb{T}(\mathbb{T}) \right )^{ \frac {1}{p}} \\ 
  &= {\lVert \phi \rVert}_\infty
\end{align}
$$
...
Moreover, if $1 \le p < q < \infty$ then 

$$
\begin{align}
{\lVert \phi \rVert}_p^p &= \int_\mathbb{T} | \phi(\omega) |^p d\lambda_\mathbb{T} (\omega) \\ 
  &= \int_{| \phi | \le 1} | \phi(\omega) |^p d\lambda_\mathbb{T} (\omega) + \int_{| \phi | > 1} | \phi(\omega) |^p d\lambda_\mathbb{T} (\omega) \\ 
  &\le \int_{| \phi | \le 1} 1 \; d\lambda_\mathbb{T} (\omega) + \int_{| \phi | > 1} | \phi(\omega) |^p d\lambda_\mathbb{T} (\omega) \\ 
  &\le \int_{| \phi | \le 1} 1 \; d\lambda_\mathbb{T} (\omega) + \int_{| \phi | > 1} | \phi(\omega) |^q d\lambda_\mathbb{T} (\omega) \\ 
  &\le \int_\mathbb{T} 1 \; d\lambda_\mathbb{T} (\omega) + \int_\mathbb{T} | \phi(\omega) |^q d\lambda_\mathbb{T} (\omega) \\ 
...
is an orthonormal set of functions in $L_2(\mathbb{T})$.  Moreover, this set is an orthonormal _basis_ of $L_2(\mathbb{T})$.  That is, each member of $L_2(\mathbb{T})$ can be approximated arbitrarily closely in the 2-norm by a finite linear combination from this set of functions.
...
Recall _Hölder's inequality_.  Given exponent $p$ such that $1 < p < \infty$ define $q = p / (p-1)$ to be the Hölder conjugate exponent of $p$.  Then 
...
Now include $(p, q) = (1, \infty)$ among the pairs of conjugate exponents.  Then for $f \in L_p(\mathbb{T})$ and $g \in L_q(\mathbb{T})$, the point-wise product $f(\theta) \times g(\theta)$ is a member of $L_1(\mathbb{T})$ and satisfies Hölder's inequality:
...
Now any measure $\mathcal{m}$ on $\mathbb{T}$ can be represented as the sum of three component measures (at least one of which is non-zero): 

  - $\mathcal{m}_a$, a component that is absolutely continuous with respect to the scaled Lebesgue measure $\lambda_\mathbb{T}$;
  - $\mathcal{m}_d$, a discrete component concentrated on a set of atoms; and 
  - $\mathcal{m}_s$, a non-atomic, continuous component that is singular with respect to $\lambda_\mathbb{T}$.

For the sake of simplicity suppose that measure $\mu$ consists of just the first two components, $\mu_a$ and $\mu_d$, with $\mu_s$ assumed to be zero.  Then for any Borel subset $E \subseteq \mathbb{T}$ we have 
...
$$
\begin{align}
  \mu_a (E) &= \int_E g_\mu(\theta_\omega) \; d\lambda_\mathbb{T} (\omega) \\
  \\ 
...
\begin{align}
  \lambda_\mu (f) &= \int_\mathbb{T} f(\theta_\omega) \; d\mu(\omega) \\ 
  &= \int_\mathbb{T} f(\theta_\omega) \; d\mu_a(\omega) \; 
...
The linear functional $\lambda_\mu$ has the norm of its corresponding measure $\mu$.
...
Note that $S_n (f)$ can be expressed as a weighted sum $S_{n, w} (f)$ by setting $\mathcal{w} (\cdot) \equiv 1$.  In this case we obtain the Dirichlet kernel function $D_n (\theta)$.

$$
\begin{align}
  D_n (\theta) &= \sum_{\nu = -n}^n e^{i \nu \theta} \\ 
  &= 1 + 2 \sum_{\nu = 1}^n \cos( \nu \theta ) \\ 
  &= \frac{ \sin \left ( (n + 1/2) \theta \right ) }{ \sin(\theta / 2) }
\end{align}
$$
...
In general, as previously noted, each functional in $\mathcal{C} (\mathbb{T})^*$ can be identified with a finite Borel measure $\mu \in \mathcal{M} (\mathbb{T})$ having an absolutely continuous component $\mu_a$ and a discrete component $\mu_d$.  (For simplicity we again assume any continuous, singular component $\mu_s$ to be zero.)  In our previous notation we have 
...
The first term $\hat{g}_\mu [\nu]$ tends to zero as $| \nu |$ increases.  If $\mu_d (\cdot) \ne 0$ then the second term generates a sequence $\{ \hat{\mu}_d [\nu] \}$ that is a sampling of a periodic or almost-periodic function that remains bounded by $\lVert \mu_d \rVert$ but does not converge to zero.
...
That is, a function $f \in \mathcal{A} (\mathbb{T})$ is continuous but may fail to have a continuous derivative at all points.  Correspondingly, a functional $\lambda \in \mathcal{PM} (\mathbb{T})$ can be evaluated at any continuously differentiable function $f \in \mathcal{C}_1 (\mathbb{T})$, but needn't be a finite Borel measure.
...
We also require the following quadratic expressions to have limiting time-averaged values.
...
If the sequence $\xi_\bullet$ satisfies these requirements then so does any translation of the sequence, $T_s (\xi_\bullet) = \xi_{\bullet + s}$, where $s$ is any fixed integer.  The limits are thus translation invariant (corresponding to a stochastic requirement that a random time series be second-order stationary).
...
Let $\mathcal{G}$ denote the set of sequences that meet these requirements.

$$
\begin{align}
  \mathcal{G} &= \left \{ \xi_\bullet: \exists \;  m_\xi, \; m_{\xi, \xi} [\cdot] \; \ni \;  \lim_{| \tau | \rightarrow \infty} m_{\xi, \xi} [\tau] = \left | m_\xi \right |^2 \right \}
\end{align}
$$

Thus $\xi_\bullet \in \mathcal{G}$ is a sequence that meets these mixing and translation-invariant requirements.

#### Corresponding measure

For $\xi_\bullet \in \mathcal{G}$ one can construct a convergent sequence of Borel measures $\{\mu_\xi^{(n)}\}$ defined on $\mathbb{T}$ whose Fourier coefficients coincide with $\{ \xi_\nu \}_{| \nu | \le n}$.

For non-negative integer $n$ and for $\theta \in (-\pi, \pi]$ we define the $n^{th}$ measure of the interval $(-\pi, \theta]$ as follows.

$$
\begin{align}
  \mu_\xi^{(n)} \; (-\pi, \theta] &= \mu_\xi^{(n)} \; (-\pi, \theta) \\ 
  &= \frac{1}{2 \pi} \left \{ (\pi + \theta) \;  \xi_0 \; + \;  \sum_{1 \le |\nu| \le n} \xi_\nu \frac{e^{i \nu \theta} - (1)^\nu}{i \nu} \right \} 
\end{align}
$$

This defines a Borel measure (absolutely contnuous with respect to $\lambda_\mathbb{T}$) such that 

$$
\begin{align}
  \lim_{\theta \downarrow - \pi} \left \{ \mu_\xi^{(n)} \; (-\pi, \theta] \right \} &= 0 \\ 
  \mu_\xi^{(n)} (\mathbb{T}) = \mu_\xi^{(n)} \; (-\pi, \pi] &= \xi_0 \\ 
...
The sequence of measures $\{\mu_\xi^{(n)}  (\cdot) \}_n$ can be shown to converge to the desired measure $\mu_\xi (\cdot)$ in the following sense.
...
is defined by the following formal integration by parts.
...
The measure $\mu_\xi (\cdot)$ also generates a Fourier decomposition of the $2^{nd}$-moment sequence $m_{\xi, \xi} [\tau]$.  This decomposition, or that of $m_{\xi, \xi} [\tau] - |m_\xi|^2$, is called the "power spectrum" of $\xi_\bullet$.  See $\S$ 3.9 of Brillinger.

In summary, for any $\xi_\bullet \in \mathcal{G}$ we can construct a Borel measure $\mu_\xi (\cdot)$ on $\mathbb{T}$ that has $\xi_\bullet$ as Fourier coefficients, as interpreted above.

#### Hilbert space

Now given another such sequence $\eta_\bullet \in \mathcal{G}$ we can form the following sequence of finite inner products.
...
One can show that this numerical sequence converges to a value, denoted $\left < \xi, \eta \right >$, that can be expressed in terms of the convolution of the respective measures $\mu_\xi (\cdot)$ and $\mu_\eta (\cdot)$.  We thus have an inner product space, and in fact a Hilbert space, say $\mathcal{H}$.

Note, however, that the squared norm in this Hilbert space $\mathcal{H}$ is 

$$
\begin{align}
  {\lVert \xi_\bullet \rVert}_{\mathcal{H}}^2 = m_{\xi, \xi} [0]
\end{align}
$$

Consequently any $\zeta_\bullet \in \mathcal{G}$ for which $m_{\zeta, \zeta} [0] = 0$ is a null sequence in $\mathcal{H}$.  Let $\mathcal{N}_\mathcal{H}$ denote the set of such sequences.

$$
\begin{align}
  \mathcal{N}_\mathcal{H} &= \left \{ \zeta_\bullet \in \mathcal{G} : m_{\zeta, \zeta} [0] = 0 \right \}
\end{align}
$$

If two sequences $\xi_\bullet, \eta_\bullet \in \mathcal{G}$ differ by a null sequence, then they are identified as the same element of $\mathcal{H}$.  Therefore, each element of $\mathcal{H}$ is an equivalence class $[\xi_\bullet]_\mathcal{H}$ of $\mathcal{G}$ sequences that differ by such null sequences (which include all of $\mathcal{l}_1$).

$$
\begin{align}
  [\xi_\bullet]_\mathcal{H} &= \left \{ \eta_\bullet \in \mathcal{G}: (\eta_\bullet - \xi_\bullet) \in \mathcal{N}_\mathcal{H} \right \}
\end{align}
$$
...
The window function $\mathcal{w} (\cdot)$ is usually defined to be an even, non-negative function such that $\mathcal{w} (0) = 1$ with $\mathcal{w} (u)$ decreasing on $u \in [0, 1]$, so as to dampen the higher harmonics of $f (\cdot)$.  Such window functions are also called "convergence factors" or "tapering functions".
...
Thus far we have considered a few simple functions, each one given by a formula.  We then derived formulas for the Fourier coefficients, which led to formulas for the corresponding Fourier approximation.  We then computed these approximations at a discrete set of $\theta$-values in order to display the original function $f (\theta)$ and its approximation $S_{n, w} (f) \; (\theta)$.
...
$$
\begin{align}
  \text{DFT} (f) \; \left ( \omega_{n_\theta}^k \right ) 
...
$$
\begin{align}
  \text{DFT} (f) \; \left ( \omega_{n_\theta}^k \right ) &\sim n_\theta \times \hat{f} [k]
\end{align}
$$
...
$$
\begin{align}
  \text{DFT} (f) \; \left ( \omega_{n_\theta}^{n_\theta - k} \right ) &= \text{DFT} (f) \; \left ( \omega_{n_\theta}^{- k} \right ) \\ 
...
In order to represent $DFT(f)$ as a matrix-vector product, we introduce the following vector notation for $\{ \theta_\nu \}$ and $\{ f(\theta_\nu) \}$.
...
$$
\begin{align}
  \Omega = \Omega_{n_\theta} &= \left ( \omega_{n_\theta}^{\mu \; \nu} \right )_{\mu, \nu = 0}^{n_\theta -  1} \\ 
  \\ 
  &= 
    \begin{pmatrix}
...
$$
\begin{align}
  \Omega^* \times \Omega &= n_\theta \times I
\end{align}
$$
...
Also note that computing a matrix-vector product $A \; v \in \mathbb{R}^n$ generally requires on the order of $n^2$ arithmetic operations, denoted $O(n^2)$.  But computing the DFT requires only $O(n \log n)$ operations due to the special structure of matrix $\Omega$ and due to the advent of the fast Fourier transform (FFT).

### Window functions

Previously we used a window function $\mathcal{w} (\cdot)$ as a convergence factor to reduce the higher-order Fourier coefficients of function $f (\cdot)$ and thereby obtain a smooth approximation $S_{n, w} (f)$ of $f (\cdot)$.  Doing so using the DFT we must keep in mind the symmetries of the unit circle.
...
We should also bear in mind that when we use the FFT, the grid point at which function $f (\theta_\nu)$ is sampled match the grid points at which $DFT (f) (\omega_{n_\theta}^\nu)$ is calculated.
...
That is, successive $m^{th}$ elements of the output vector correspond to successive Fourier coefficients of $f (\cdot)$.  The remaining elements equal zero (within rounding error).
...
This technical note is intended to serve as a brief refresher on Fourier series in preparation for a subsequent note on the Fourier analysis of time series data.  To that end mathematical foundations are outlined, culminating in Wiener's Generalized Harmonic Analysis (GHA), which defines translation invariance and mixing requirements of numeric sequences.  The note concludes with example functions whose Fourier approximations are presented as convolutions with the Dirichlet kernel function $D_n (\cdot)$ and with the Fejér kernel function $F_n (\cdot)$, respectively.
